...
export const CreatePurchaseOrderPageV2: React.FC = () => {
  ...
  const addItem = () => {
    if (!newItem.materialId || newItem.quantity <= 0 || newItem.unitPrice <= 0) {
    ...
    setNewItem({ materialId: '', materialName: '', quantity: 0, unitPrice: 0 });
    setError('');
  };
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) => {
                    ...
                    setNewItem({ ...newItem, materialId: e.target.value, materialName: material?.name || '' });
                  }}
                  style={{ width: '100%', padding: '0.75rem', border: '2px solid var(--border)' }}
                >
              ...
              <tbody>
                {form.items.map((item, index) => (
                  <tr key={index} style={{ borderBottom: '1px solid var(--border)' }}>
                    <td style={{ padding: '1rem' }}>{item.materialName}</td>
